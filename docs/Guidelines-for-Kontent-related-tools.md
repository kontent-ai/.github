...
Before you publicly offer any tool, it is recommended to read through the [Kontent Github checklist for publishing an OS project](https://github.com/kontent-ai/.github/wiki/Checklist-for-publishing-a-new-OS-project) to be aware of all enhancements you could implement to make the project more credible and well-perceived by the community.

### Analytics

To be able to better measure the adoption of your project, the tool should include the `X-KC-SOURCE` header with each request sent to Kontent API ([Delivery](https://docs.kontent.ai/reference/delivery-api), [Management](https://docs.kontent.ai/reference/management-api-v2), and [Assets](https://docs.kontent.ai/reference/image-transformation)). The value of the header should be formatted as follows: `<Tool identification>;<Tool version>`. For example, `Acme.Kontent.AwesomeTool;1.0.0` or, for [Kontent Sourcebit source plugin](https://www.npmjs.com/package/@kontent-ai/sourcebit-source-kontent/v/0.1.42), `@kontent-ai/sourcebit-source-kontent;0.1.42`. The version itself is not required, you can use just `Tool identification` like [kontent-nuxt-module](https://github.com/Domitnator/kentico-kontent-nuxt-module/issues/10). However, we strongly recommend including the version to be able to track adoption among multiple versions.
